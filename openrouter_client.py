...
async def analyze_chart(image_bytes: bytes, caption: str = "") -> str:
    ...
    prompt = CHART_ANALYSIS_PROMPT.replace("{moon_phase}", moon["phase"]).replace("{moon_illumination}", str(moon["illumination"])).replace("{pair}", caption.split()[0] if caption.split() else "BTC").replace("{MOON_INSIGHT}", moon_advice)
